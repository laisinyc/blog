...
- **content-box**: Default value, will add border and padding as part of width and height
...
// Solution 1: add a block element after float elements
<div class="form-item">
  <button class="left" disabled="disabled">Register</button>
  <button class="right">Reset</button>
  <span style="
...
改变 `span` 的文字大小 会导致 `checkbox` 和 `span` 水平上错位，如何修正？

```html
<!-- 当水平方向上有多个 inline, inline-block 元素时容易出现错位 -->
...
- 定位类型：static(默认值)、relative、absolute、fixed 和 sticky
- **absolute**: first positioned element
...
<PdfViewer file="../public/pdf/属性值计算过程简介.pdf" />
